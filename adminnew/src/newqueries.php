...
if (strlen(trim($id))>0) {
    ...
    if ($conn->query($sql)) {
        echo "<script>alert('Query Deleted');location.href='newqueries'</script>";
    }
}
?>
<head>
                                ...
                                <div class="card-body">
                                    ...
                                    <table id="scroll-horizontal-datatable" class="table table-striped  nowrap table-striped w-100" >
                                        <thead>
                                            ...
                                            <?php
                                                $query = mysqli_query($conn, "SELECT * FROM `queries` WHERE `description`='' or `description` IS NULL and type='nutritionist' and `status`=1 ORDER BY `id` DESC");
                                                $sno = 0;
                                                while ($fetch = mysqli_fetch_array($query)) { ?>
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <a class="btn btn-sm btn-outline-primary rounded-pill" href="replyQuery?id=<?php echo $fetch['id'] ?>&backTo=newqueries">Edit</a>
