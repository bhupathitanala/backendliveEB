                                    ...
                                    <table id="scroll-horizontal-datatable"
                                        ...
                                        <thead>
                                            ...
                                            <?php
                                                ...
                                                $query = mysqli_query($conn, "SELECT a.order_id,a.order_placed_date, GROUP_CONCAT(a.product_id) as productids,GROUP_CONCAT(DISTINCT a.invoice_number) as invoice_number, GROUP_CONCAT(DISTINCT a.customer_address) as customer_address, (SELECT d.email FROM admin_users d WHERE id IN (SELECT b.user_id FROM vendors b WHERE b.vendorID IN(SELECT c.vendor_id FROM productsnew c WHERE c.ID=a.product_id ))) as vendor_email,(SELECT d.name FROM admin_users d WHERE id IN (SELECT b.user_id FROM vendors b WHERE b.vendorID IN(SELECT c.vendor_id FROM productsnew c WHERE c.ID=a.product_id ))) as vendor_name FROM order_details a where a.status=1 GROUP by vendor_email,vendor_name,a.order_id,a.order_placed_date order by a.order_placed_date desc");
                                                $sno = 0;
                                                while ($fetch = mysqli_fetch_array($query)) { 
                                                    $sql = mysqli_query($conn, "SELECT * from `order_details` WHERE order_id='".$fetch['order_id']."' and invoice_number='".$fetch['invoice_number']."'");
                                                    ...
                                                    <tr>
                                                        ...
                                                        $values = explode(",", $fetch['productids']);

                                                        // Count occurrences of each value
                                                        $valueCounts = sizeof(array_count_values($values));
                                                        echo $fetch['productids'].'('.($valueCounts).')'; ?></td>
                                                        <td><?php echo $fetch['order_placed_date']; ?></td>
                                                        <td><?php echo $row['payment_status']; ?></td>
                                                        <td>
                                                            <button class="btn btn-sm btn-primary view-btn" data-row-data="<?php echo htmlentities(json_encode($row)); ?>"><i class="fa fa-eye"></i></button>
                                                        </td>
                                                        <td><?php echo $row['payment_mode']; ?></td>
                                                        <td><i class="bi bi-currency-rupee"></i><?php echo $row['amount']." /-"; ?></td>
                                                        ...
                                                        <td>
                                                            <?php if ($row['delivered_status']==1) { ?>
                                                                <label class="badge" style="background: green;">Delivered</label>
                                                            <?php } else if($row['shipped_status']==1) { ?>
                                                            ...
                                                            <?php } else if($row['confirmed_status']==1) { ?>
                ...
                if (status!="") {
                    ...
                    $.ajax({
                        ...
                        data: JSON.stringify({
                            ...
                            // "vendorEmail":'rksimhadri731@gmail.com',
                            "userEmail": userEmail,
                            "userType": userType
                        }),
                        method: "POST",
