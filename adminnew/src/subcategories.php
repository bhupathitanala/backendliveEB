...
if (strlen(trim($id))>0) {
    ...
    if ($conn->query($sql)) {
        echo "<script>alert('Sub category Deleted');location.href='subcategories'</script>";
    }
}
?>
<head>
                            ...
                            <div class="card">
                                ...
                                <div class="card-body">
                                    ...
                                    <table id="fixed-header-datatable"
                                        ...
                                        <tbody>
                                            <?php
                                                $query = mysqli_query($conn, "SELECT *, (SELECT `categoryName` FROM `categories` WHERE `id` = a.`mcID`) AS `categoryName` FROM `subcategory` a ORDER BY `mcID` ASC");
                                                $sno = 0;
                                                while ($fetch = mysqli_fetch_array($query)) { ?>
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <a class="btn btn-sm btn-outline-primary rounded-pill" href="addsubcategory?id=<?php echo $fetch['ID'] ?>">Edit</a>
                                                            <a class="btn btn-sm btn-outline-danger rounded-pill" href="?id=<?php echo $fetch['ID'] ?>" onclick="return confirm('Are you sure to delete this sub category')">Delete</a>
                                                        </td>
                                                    </tr>
                                            <?php } ?>
                                        </tbody>
