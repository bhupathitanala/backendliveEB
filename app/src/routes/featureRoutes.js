...
router.post('/', upload.single('image'), async (req, res) => {
    try {
        ...
        await pool.query('INSERT INTO features (title, status, icon) VALUES (?, ?, ?)',
            [categoryName, status, img_url], async function (error, result) {
                if (error) {
                    ...
                    pool.query('SELECT * FROM features WHERE id = ?', [insertedId], function (selectError, selectResult) {
                        if (selectError) {
...
router.get('/', async (req, res) => {
    try {
        await pool.query('SELECT * FROM features where status = 1', function (error, result, fields) {
            if (error) {
                console.error(error.message);
                res.status(500).json({ message: 'Internal server error' });
            };
...
router.get('/getall', async (req, res) => {
    try {
        await pool.query('SELECT * FROM features', function (error, result, fields) {
            if (error) {
                console.error(error.message);
                res.status(500).json({ message: 'Internal server error' });
            };
...
router.get('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        await pool.query('SELECT * FROM features WHERE id = ?', [id], async function (error, data) {
            if (error) {
                console.error(error.message);
                res.status(500).json({ message: 'Internal server error' });
            };
...
router.put('/:id', upload.single('image'), async (req, res) => {
    try {
        ...
        let statement = 'UPDATE features SET title = ?, status = ? WHERE id = ?';
        let values = [categoryName, status, id]

        // If an image is provided, delete the old image associated with the category
        if (img_url) {
            await pool.query('SELECT icon FROM features WHERE id = ?', [id], async function (error, oldCategory) {
                if (error) {
                    console.error(error.message);
            ...
            statement = 'UPDATE features SET title = ?, status = ?, icon = ? WHERE id = ?';
        ...
        await pool.query(
            ...
            function (error, result) {
                ...
                if (result.affectedRows > 0) {
                    ...
                    pool.query('SELECT * FROM features WHERE id = ?', [id], function (selectError, selectResult) {
                        if (selectError) {
                            console.error(selectError.message);
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        pool.query('DELETE FROM features WHERE id = ?', [id], function (categoryDeleteError, categoryDeleteResult) {
