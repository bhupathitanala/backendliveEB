...
router.post('/neworder', async (req, res) => {
    try {  
        ...
        const { payment_status, payment_type, shipping_charges, cod_charges, amount, coupon_code, payment_mode, customer_id, customer_name, customer_email, customer_contact, delivery_status,cart_type, customer_address, product_details, productswithqty, razorpay_details } = req.body;       
...
router.post('/neworderforguest', async (req, res) => {
    try {  
        ...
        const { payment_status, payment_type, shipping_charges, cod_charges, amount, coupon_code, payment_mode, customer_type, customer_name, customer_email, customer_contact, delivery_status, customer_address, product_details, productswithqty, razorpay_details } = req.body;       
        // Get current date and time in Kolkata timezone
        const order_placed_date = moment().tz('Asia/Kolkata').format('YYYY-MM-DD HH:mm:ss');
        const cus_details = JSON.stringify(customer_address)
        const pr_details = JSON.stringify(product_details);
        ...
        for (const product of productswithqty) {
            ...
            await pool.query('INSERT INTO order_details (order_id, invoice_number, payment_status, payment_type, shipping_charges, cod_charges, amount, coupon_code, payment_mode,  customer_type, customer_name, customer_email, customer_contact, delivery_status, customer_address, product_details, product_id, product_title, product_qty,product_type, product_price, product_img, razorpay_details, order_placed_date, ordered_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
            [order_id, invoice_number, payment_status, payment_type, shipping_charges, cod_charges, amount, coupon_code, payment_mode, customer_type, customer_name, customer_email, customer_contact, delivery_status, cus_details, pr_details, productID, product_title, qty, product_type, product_price, product_img, razorpay_details, order_placed_date, order_placed_date], async function (productError) {
                if (productError) {
...
//         const response = await axios.post('http://3.7.47.11/backendlive/adminnew/src/emailTemplates/order_confirm_customer.php', req.body);
...
//         // Fetch HTML template from the PHP file
//         const response = await axios.post('http://3.7.47.11/backendlive/adminnew/src/emailTemplates/Admin_NewOrder.php', req.body);
        ...
        if(status === 'Confirmed'){
            ...
            const confirmed_date = moment().tz('Asia/Kolkata').format('YYYY-MM-DD HH:mm:ss');
                await pool.query(
                    ...
                    [updatedstatus, confirmed_date, delivery_status, orderId, invoice_number], function (error, result) {
                        ...
                        axios.post('http://3.7.47.11/backendlive/adminnew/src/emailTemplates/order_processing.php', req.body)
                        .then(response => {
                            const htmlTemplate = response.data;
            ...
            await pool.query(
                ...
                [updatedstatus, shipped_date, delivery_status, orderId, invoice_number], function (error, result) {
                    ...
                    req.body.title = subject;
                    axios.post('http://3.7.47.11/backendlive/adminnew/src/emailTemplates/order_status.php', req.body)
                            .then(response => {
                                const htmlTemplate = response.data;
                                //console.log(htmlTemplate); // Check if the response is not empty
                            ...
                            .then(() => {
                                ...
                                //Admin Email
                                return axios.post('http://3.7.47.11/backendlive/adminnew/src/emailTemplates/order_sharing.php', req.body)
                                        .then((response1) => {
            ...
            await pool.query(
                ...
                [updatedstatus, delivered_date, delivery_status, orderId, invoice_number], function (error, result) {
                    ...
                    req.body.title = subject;
                    axios.post('http://3.7.47.11/backendlive/adminnew/src/emailTemplates/order_status.php', req.body)
                    // axios.post('http://3.7.47.11/backendlive/adminnew/src/emailTemplates/order_delivered.php', req.body)
                            .then(response => {
                                const htmlTemplate = response.data;
                                //console.log(htmlTemplate); // Check if the response is not empty
                            ...
                            .then(() => {
                                ...
                                //Admin Email
                                return axios.post('http://3.7.47.11/backendlive/adminnew/src/emailTemplates/order_sharing.php', req.body)
                                // return axios.post('http://3.7.47.11/backendlive/adminnew/src/emailTemplates/Admin-OrderCompleted.php', req.body)
                                    .then((response1) => {
                                        ...
                                        // return axios.post('http://3.7.47.11/backendlive/adminnew/src/emailTemplates/Vendor-OrderCompleted.php', req.body)
                                        .then((response1) => {
...
//         const response = await axios.post('http://3.7.47.11/backendlive/adminnew/src/emailTemplates/order_confirm_customer_all.php', record);
//         const htmlTemplate = response.data;
...
//         // Fetch HTML template from the PHP file
//         const response1 = await axios.post('http://3.7.47.11/backendlive/adminnew/src/emailTemplates/Admin_NewOrder.php', record);
...
const sendOrderConfirmationEmailwithAttachments = async (order_id, customer_email) => {
    ...
    try {
        ...
        const result = await queryAsync('SELECT GROUP_CONCAT(a.product_id) as productids,GROUP_CONCAT(DISTINCT a.invoice_number) as invoice_number, GROUP_CONCAT(DISTINCT a.customer_address) as customer_address, (SELECT d.email FROM admin_users d WHERE id IN (SELECT b.user_id FROM vendors b WHERE b.vendorID IN(SELECT c.vendor_id FROM productsnew c WHERE c.ID=a.product_id ))) as vendor_email FROM order_details a WHERE order_id=? GROUP by vendor_email', [order_id]);
        ...
        for (const row of result) {
            ...
            const file_path = 'http://3.7.47.11/backendlive/adminnew/src/emailTemplates/invoice/' + file_name;
            ...
            const response = await axios.post('http://3.7.47.11/backendlive/adminnew/src/emailTemplates/order_invoice.php', row);
            const htmlTemplate = response.data;
            // console.log(htmlTemplate)
       
            const mpdfData = { html: htmlTemplate, order_id: order_id, invoice_number: row['invoice_number']};

            const response1 = await axios.post('http://3.7.47.11/backendlive/adminnew/src/emailTemplates/generate_invoice.php', mpdfData);
            //const htmlTemplate1 = response1.data;
            ...
            const vendorTemplateResponse = await axios.post('http://3.7.47.11/backendlive/adminnew/src/emailTemplates/Vendor_NewOrderPlaced.php', row);
        ...
        const customerTemplateResponse = await axios.post('http://3.7.47.11/backendlive/adminnew/src/emailTemplates/order_confirm_customer_all.php', record);
        ...
        const adminTemplateResponse = await axios.post('http://3.7.47.11/backendlive/adminnew/src/emailTemplates/Admin_NewOrder.php', record);
...
const sendOrderConfirmationEmailwithAttachment = async (record) => {
    try {
        // Generate the invoice template
        const response = await axios.post('http://3.7.47.11/backendlive/adminnew/src/emailTemplates/order_invoice.php', record);
        const htmlTemplate = response.data;
        ...
        // Email Templates
        const response2 = await axios.post('http://3.7.47.11/backendlive/adminnew/src/emailTemplates/order_confirm_customer_all.php', record);
        ...
        let file_path = 'http://3.7.47.11/backendlive/adminnew/src/emailTemplates/invoice/'+record.invoice_number+'.pdf';
