    ...
    try {
        ...
        upload(req, res, async function (err) {
            ...
            try {
                ...
                await pool.query('INSERT INTO blogs (title, description, heading, subHeading, img_urls) VALUES (?, ?, ?, ?, ?)',
                    [title, description, heading, subHeading, JSON.stringify(img_urls)], function (error, result) {
                        if (error) {
                            ...
                            pool.query('SELECT * FROM blogs WHERE id = ?', [insertedId], function (selectError, selectResult) {
                                if (selectError) {
...
router.get('/', async (req, res) => {
    try {
        let statement = 'SELECT * FROM blogs';
...
router.get('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        await pool.query('SELECT * FROM blogs WHERE id = ?', [id], function (error, data) {
            if (error) {
                console.error(error.message);
                res.status(500).json({ message: 'Internal server error' });
            };
...
router.get('/hot/:type', async (req, res) => {
    try {
        let { type } = req.params;
        const statement = "SELECT * FROM blogs where is_hot = ?";
...
router.post('/hot/updateblogs', async (req, res) => {
    const selectedProductIds = req.body;

    try {
        const updateSelectedblogsSQL = `UPDATE blogs SET is_hot = '1' WHERE id IN (?)`;
        await pool.query(updateSelectedblogsSQL, [selectedProductIds]);

        // Update other products in MySQL
        const updateOtherBlogsSQL = `UPDATE blogs SET is_hot = '0' WHERE id NOT IN (?)`;
        await pool.query(updateOtherBlogsSQL, [selectedProductIds]);
...
router.get('/trending/:type', async (req, res) => {
    try {
        let { type } = req.params;
        const statement = "SELECT * FROM blogs where is_trending = ?";
        await pool.query(statement, [type], async function (error, result, fields) {
            if (error) {
                console.error(error.message);
                res.status(500).json({ message: 'Internal server error' });
...
router.post('/hot/updateblogs', async (req, res) => {
    ...
    try {
        const updateSelectedblogsSQL = `UPDATE blogs SET is_trending = '1' WHERE id IN (?)`;
        await pool.query(updateSelectedblogsSQL, [selectedProductIds]);

        // Update other products in MySQL
        const updateOtherBlogsSQL = `UPDATE blogs SET is_trending = '0' WHERE id NOT IN (?)`;
        await pool.query(updateOtherBlogsSQL, [selectedProductIds]);
    ...
    upload(req, res, async function (err) {
        ...
        try {
            ...
            if (img_urls.length > 0) {
                ...
                pool.query('SELECT img_urls FROM blogs WHERE id = ?', [id], async function (error, oldProduct) {
                    if (error) {
                        console.error(error.message);
                        return res.status(500).json({ message: 'Internal server error' });
                    }
            ...
            let updateQuery = 'UPDATE blogs SET  heading = ?, title = ?, description = ?, subHeading = ?';
            ...
            pool.query(
                ...
                async function (error, result) {
                    ...
                    if (result.affectedRows > 0) {
                        ...
                        pool.query('SELECT * FROM blogs WHERE id = ?', [id], function (selectError, selectResult) {
                            if (selectError) {
                                console.error(selectError.message);
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        pool.query('SELECT img_urls FROM blogs WHERE id = ?', [id], async (selectError, productData) => {
            if (selectError) {
                console.error(selectError.message);
                return res.status(500).json({ message: 'Internal server error' });
            }
            ...
            pool.query('DELETE FROM blogs WHERE id = ?', [id], (deleteError, deleteResult) => {
                if (deleteError) {
