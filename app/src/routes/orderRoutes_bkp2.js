...
const cronjobsdata = async () => {
    ...
    try {
        ...
        const result = await queryAsync('SELECT distinct invoice_number,date(ordered_date) as ordered_date,payment_type,customer_address,customer_name,customer_email FROM order_details WHERE delivery_status = ?', [delivery_status]);
...
const sendOrderConfirmationEmailwithAttachment = async (record) => {
    try {
        ...
        const response = await axios.post('http://3.7.47.11/backnew/adminnew/src/emailTemplates/order_invoice.php', record);
        const htmlTemplate = response.data;
       
        const mpdfData = { html: htmlTemplate, invoice_number: record.invoice_number};

        const response1 = await axios.post('http://3.7.47.11/backnew/adminnew/src/emailTemplates/generate_invoice.php', mpdfData);
        // const htmlTemplate1 = response1.data;



        // Email Templates
        const response2 = await axios.post('http://3.7.47.11/backnew/adminnew/src/emailTemplates/order_confirm_customer_all.php', record);
        ...
        let file_path = 'http://3.7.47.11/backnew/adminnew/src/emailTemplates/invoice/'+record.invoice_number+'.pdf';
        ...
        for (const row of result) {
            ...
            // Email Templates
            const vendorTemplateResponse = await axios.post('http://3.7.47.11/backnew/adminnew/src/emailTemplates/Vendor_NewOrderPlaced.php', row);
...
const cronjobsdatatesting = async () => {
    ...
    try {
        const invoice_number = 'EBIN000922';
        const result = await queryAsync('SELECT GROUP_CONCAT(a.product_id) as productids,(SELECT d.email FROM admin_users d WHERE id IN (SELECT b.user_id FROM vendors b WHERE b.vendorID IN(SELECT c.vendor_id FROM productsnew c WHERE c.ID=a.product_id ))) as vendor_email FROM order_details a WHERE invoice_number=? GROUP by vendor_email', [invoice_number]);
